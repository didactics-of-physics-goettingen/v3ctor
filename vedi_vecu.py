...
## -------- Berechnung der Integrale
    ...
    def calc_AreaInt(self):
        ...
        print('Used Koord fpr Intergal:', x0 , x1 ,y0 , y1)
...
## -------- Berechnen der Divergenz/Rotation an einem Punkt
    ...
    def rightclick(self,event):
        ...
        if self.integral_kind.get() == 'Satz von Gauss' or self.integral_kind.get() =="Gauss' theorem" :
            self.print_divergence(cordx,cordy)
        elif self.integral_kind.get() == 'Satz von Stokes' or self.integral_kind.get() =="Stokes' theorem":
            self.print_curl(cordx,cordy)


    def print_divergence(self,cordx,cordy):
        x,y = self.Transform(cordx,cordy)
        Fx=self.F[0].evalf(5, subs={self.x: x, self.y:y})*self.scale
        Fy=self.F[1].evalf(5,subs={self.x: x, self.y:y})*self.scale
        print('Cords: ',x,y)
    ...
    def print_curl(self,cordx,cordy):
        x,y = self.Transform(cordx,cordy)
        sy_curl=self.curl()
    ...
    def draw_line_arrows(self):
        ...
        if self.check_var_RectVec.get() == True:
            ...
            for i in range(int(x1),int(x2),int(self.max_arrow_size)):
               ...
               # cordx_d,cordx_u,cordy_d,cordy_u = (x1_d-(self.canvas_size/2),x1_u-(self.canvas_size/2),y_d-(self.canvas_size/2),y_u-(self.canvas_size/2))
            ...
            #    cordx_l,cordx_r,cordy_l,cordy_r = (x_l-(self.canvas_size/2),x_r-(self.canvas_size/2),y1_l-(self.canvas_size/2),y1_r-(self.canvas_size/2))
...
app.master.iconphoto(True, tk.PhotoImage(file='unilogo.png'))
